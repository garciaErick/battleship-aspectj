...
public class BattleshipDialog extends JDialog {
    ...
    private JPanel makeBoardPane() {
    	return new BattleBoardPanel(board);
    }
    
    /** Place ships randomly. */
    private void placeShips() {
        int size = board.size();
        for (Battleship ship : board.ships()) {
            int i = 0;
            int j = 0;
            boolean dir = false;
            do {
    ...
    private BattleBoard.BoardChangeListener createBoardChangeListener() {
    	return new BattleBoard.BoardChangeAdapter() {
            public void hit(Place place, int numOfShots) {
                showMessage("Shots: " + numOfShots);
            }

            public void gameOver(int numOfShots) {
                showMessage("All ships destroyed in " + numOfShots + " shots!");
            }
        }; 
    }
